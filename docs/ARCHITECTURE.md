...
Scenario replay version 5 pins replay kind/version, scenario ID/version, station-grid ID/version, RNG algorithm/version/seed, target night count, command stream, and stop tick. It validates the complete envelope against an injected scenario definition before initialization, executes time, job, retail-price, and inventory commands by tick and sequence, and reports consumed/unconsumed command IDs, receipts, final RNG, an explicit stop reason, full state, and independent state/ledger diagnostic hashes. Repeated runs compare full state and ledger rather than trusting hashes alone, including attributed service and canonical RNG consumption. The GS-010 clock replay remains an explicit version-1 compatibility adapter and rejects later command variants.
...
Checkpoint version 7 includes scenario identity, complete RNG continuation state, canonical station occupancy, workforce positions, canonical service skills, activities, business prices, attributed active-service performance, the full ledger, and sequence cursor; it deeply detaches nested snapshots, canonicalizes object keys, preserves semantic array order, and excludes presentation copy. Checkpoint creation validates active job references, route bounds/blocking/continuity/cursor, interaction destinations, durations, assignment uniqueness, customer identity/demand/stage, exact performance formulas/draw ordering/interruption history, price bounds, and business/resource ledger reconciliation against the injected scenario context. Checkpoints and replays remain deterministic diagnostics; save schema v3 embeds checkpoint v7 behind its own strict compatibility and semantic load boundary.
...
GS-015/020/021 define canonical UTF-8 JSON save schema v3 under the generic `station-campaign-save` format ID. The envelope separates minimal campaign progress, checkpoint-v7 station state, the next runtime command sequence, versioned settings/difficulty namespaces, explicit scenario/grid/RNG compatibility, and recovery metadata. Set-like campaign IDs are unique and lexical; event, path, customer, skill, and RNG arrays retain semantic order. Player-facing title, UI/modal state, rendering objects, wall-clock timestamps, runner debt, receipts, and localized event prose are excluded. Retained v1/checkpoint-v5 and v2/checkpoint-v6 decoders migrate scenario v3/v4 saves forward. V1 removes retired placeholder daytime flows and initializes future traffic; V2 authors canonical employee skills and requeues any active legacy service at an explicit future performance baseline so migration never fabricates prior modifier or RNG facts.
