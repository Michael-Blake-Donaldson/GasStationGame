...
- [x] **GS-021 (P0): Implement employee skill and fatigue.** Acceptance met: scenario-authored pump/checkout skills and exact fatigue tiers change integer service duration and canonical-RNG rework risk; service-start facts snapshot the complete arithmetic, draw cursor, attributed worker, and outcome; the shift board exposes base values, skill reductions, fatigue penalties, final duration/chance, and active roll/rework. Clearing an assignment emits an interruption and requeues unfinished service, preventing invisible double work. Scenario/replay v5, checkpoint v7, and save v3 preserve exact continuation; frozen v1/v2 fixtures and a mid-service v2 migration remain supported.
- [x] **GS-022 (P0): Implement construction rules.** Acceptance met: content-authored main-store/garage facilities use reserved compatible plots, while wall, gate, floodlight, turret, ammo storage, repair station, and generator upgrade use flexible footprints with quarter-turn rotation and provisional exact cash/scrap costs. One pure evaluator powers UI preview and authoritative dispatch, reports every grid/resource/crew-route blocker, and rejects stale placements without mutation. Canonical placement identities, resource deltas, occupancy, replay, checkpoint v8, save v4, retained v1-v3 migrations, an accessible construction modal, and authoritative scene shells are verified.
- [x] **GS-023 (P0): Validate paths after construction.** Acceptance met: Great Plains content declares a checkout-anchored required work network covering the Beacon, pumps, garage plot, checkout, and every current employee. The shared construction evaluator rejects the segment that blocks a final interaction cell or disconnects a crew/work endpoint, names the exact target or employee and cells, preserves active routes, and applies no cost/occupancy mutation. Replay, checkpoint/save semantic validation, a safe scenario-v6 save migration, modal copy, and solid provisional gate behavior are verified.
